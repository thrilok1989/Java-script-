...
def _send_telegram_dual(message: str, parse_mode: str = "Markdown") -> Tuple[bool, str]:
    """Helper function to send Telegram message to both configured recipients"""
    try:
        from telegram_alerts import TelegramBot
        bot = TelegramBot()
        if not bot.enabled:
            return False, "Telegram not configured"
        success = bot.send_message(message, parse_mode=parse_mode)
        if success:
...
def send_ml_signal_telegram(signal, spot_price: float = None):
    """
    Send ML Signal to Telegram when BUY/SELL signal is generated (dual recipient support)
    """
    try:
...
"""
        ...
        # Send to Telegram using dual-recipient helper
        success, msg = _send_telegram_dual(message, "Markdown")
...
def send_reversal_entry_telegram(
    ...
    """
    Send Telegram alert when price enters EXACT REVERSAL zone (dual recipient support)
...
def send_sl_hunt_telegram(result, spot_price: float):
    """
    Send SL Hunt Alert to Telegram when high probability hunt is detected (dual recipient support)
